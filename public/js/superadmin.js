...
function renderCategoryView(el, cat) {
  const filtered = allStats.filter(
    (c) => c.status === "ok" && c.category === cat,
  );
  if (!filtered.length) {
    el.innerHTML =
      '<div class="empty"><i class="ti ti-building-off"></i>এই category-তে কোনো center নেই।</div>';
    return;
  }
  el.innerHTML = `
    <div class="kpi-grid">
      ...
      ${centers
        ...
        <div class="h-card cat${c.category || "B"}">
          ...
          <div class="h-card-actions">
            ...
            ${
              isDir
                ? `<button class="h-btn" onclick="openEditModal(${c.id})"><i class="ti ti-edit"></i> সম্পাদনা</button>
...
function showDTab(tab, btn) {
  ...
  if (tab === "dEmployee") {
    renderEmployeeTab(
      currentDetailSlug,
      currentDetailData?.center?.category || "B",
    );
    return;
  }
  renderDetailTab(tab);
...
function renderDetailTab(tab) {
  ...
  if (tab === "dSales") {
    ...
    el.innerHTML = `
      ...
      ${
        userRole !== "director"
          ? `<div class="card"><div class="card-head">🧾 সাম্প্রতিক চালান</div>
          ...
          ${
            d.users
              .map(
                (
                  u,
                ) => `<div class="user-item"><div class="user-av" style="background:${roleColor(u.role)}">${u.name.charAt(0).toUpperCase()}</div>
    ...
    const curFY =
      new Date().getMonth() >= 6
        ...
        : new Date().getFullYear() - 1; // FY start year
    ...
    const lTA = async (fy) => {
      ...
      try {
        const r = await fetch(
          `${API}/center/${currentDetailSlug}/targets?fy=${fy}&_t=${Date.now()}`,
          {
            headers: {
        ...
        if (!data.success) {
          document.getElementById("saTargetResults").innerHTML =
            `<div class="lt">${data.error || "Error"}</div>`;
          return;
        }
        ...
        const annualProd = prodT.find(
          (t) => parseInt(t.target_month || 0) === 0,
        );
        const annualSales = salesT.find(
          (t) => parseInt(t.target_month || 0) === 0,
        );
        const tProd = annualProd
          ? parseInt(annualProd.target_quantity || 0)
          : prodT.reduce(
              (s, t) =>
                parseInt(t.target_month) > 0
                  ? s + parseInt(t.target_quantity || 0)
                  : s,
              0,
            );
        const tSales = annualSales
          ? parseFloat(annualSales.target_amount || 0)
          : salesT.reduce(
              (s, t) =>
                parseInt(t.target_month) > 0
        ...
        const pp =
          tProd > 0
            ? Math.min(Math.round((prod_achieved / tProd) * 100), 100)
            : 0;
        const sp =
          tSales > 0
            ? Math.min(Math.round((sales_achieved / tSales) * 100), 100)
            : 0;
        const pc = (p) =>
          p >= 70 ? "var(--green)" : p >= 40 ? "var(--amber)" : "var(--red)";
        document.getElementById("saTargetResults").innerHTML = `
          ...
          ${
            ...
            <tbody>${targets
              ...
              : '<div class="empty" style="padding:20px">এই অর্থবছরে কোনো লক্ষ্যমাত্রা নেই</div>'
          }`;
      } catch (e) {
        document.getElementById("saTargetResults").innerHTML =
          `<div class="lt">লোড হয়নি।</div>`;
      }
    };
    ...
    const initHTML =
      preloaded && preloaded.targets
        ? renderFYHTML(
            preloaded.targets,
            preloaded.prod_achieved,
            preloaded.sales_achieved,
          )
        : '<div class="lt">লোড হচ্ছে...</div>';
    ...
    el.innerHTML = `
      ...
      ${
        ...
        <tbody>${dm.by_reason
          ...
          : '<div class="empty"><i class="ti ti-plant-off"></i>কোনো ক্ষতির তথ্য নেই</div>'
      }`;
  } else if (tab === "dIncome") {
    ...
    el.innerHTML = `
      ...
      ${
        ...
        <tbody>${oi.breakdown
          ...
          : '<div class="empty"><i class="ti ti-cash-off"></i>কোনো অন্যান্য আয় নেই</div>'
      }`;
  }
}
...
async function renderEmployeeTab(slug, category) {
  ...
  try {
    ...
    const filledPermanent = d.permanent.filter(
      (e) => e.status === "active",
    ).length;
    const vacant = Math.max(0, totalSanc - filledPermanent);

    // designation count map
    const desigMap = {};
...
function renderDistrictSummaryView(el) {
  const ok = allStats.filter((c) => c.status === "ok");
  if (!ok.length) {
    el.innerHTML = '<div class="empty">Data নেই।</div>';
    return;
  }
  ...
  const byDivision = {};
  ok.forEach((c) => {
    const d = c.district || "অজানা";
    const dv = c.division || "অজানা";
  ...
  const districtCards = Object.entries(byDistrict)
    .sort((a, b) => b[1].centers.length - a[1].centers.length)
    .map(([dist, data]) => {
      ...
      const avgScore = Math.round(
        cs.reduce((s, c) => s + c.perf_score, 0) / cs.length,
      );
      return `
  ...
  // Division summary
  const divisionSummary = Object.entries(byDivision)
    .sort((a, b) => b[1].centers.length - a[1].centers.length)
    .map(([div, data]) => {
      const cs = data.centers;
      const rev = cs.reduce((s, c) => s + c.total_revenue, 0);
  ...
  window._dView = (type) => {
    document.getElementById("view-district").style.display =
      type === "district" ? "block" : "none";
    document.getElementById("view-division").style.display =
      type === "division" ? "block" : "none";
    document.getElementById("btn-district").style.background =
      type === "district" ? "var(--accent)" : "var(--card)";
    document.getElementById("btn-district").style.color =
      type === "district" ? "#fff" : "var(--muted)";
    document.getElementById("btn-district").style.border =
      type === "district" ? "none" : "1px solid var(--border)";
    document.getElementById("btn-division").style.background =
      type === "division" ? "var(--accent)" : "var(--card)";
    document.getElementById("btn-division").style.color =
      type === "division" ? "#fff" : "var(--muted)";
    document.getElementById("btn-division").style.border =
      type === "division" ? "none" : "1px solid var(--border)";
  };
}
...
function renderTargetSummaryView(el) {
  ...
  const totalMonthlyTarget = ok.reduce(
    (s, c) => s + (c.monthly_prod_target || 0),
    0,
  );
  const totalMonthlyAchieved = ok.reduce(
    (s, c) => s + (c.monthly_prod_achieved || 0),
    0,
  );
  const overallPct =
    totalMonthlyTarget > 0
      ...
      : 0;
  const centersOnTrack = ok.filter(
    (c) =>
      c.monthly_prod_target > 0 &&
      c.monthly_prod_achieved / c.monthly_prod_target >= 0.7,
  ).length;
  el.innerHTML = `
    <div class="kpi-grid" style="margin-bottom:16px">
          ...
          .map((c) => {
            const pct =
              c.monthly_prod_target > 0
                ? Math.min(
                    Math.round(
                      (c.monthly_prod_achieved / c.monthly_prod_target) * 100,
                    ),
                    200,
                  )
                : null;
...
function renderCompareView(el) {
  ...
  if (!ok.length) {
    el.innerHTML =
      '<div class="empty"><i class="ti ti-chart-off"></i>Data নেই।</div>';
    return;
  }

  // Filter options
  const districts = [
    ...new Set(ok.map((c) => c.district).filter(Boolean)),
  ].sort();
  const divisions = [
    ...new Set(ok.map((c) => c.division).filter(Boolean)),
  ].sort();
...
function renderCompareTable(data) {
  ...
  document.getElementById("compareCount").textContent =
    data.length + "টি center";
  window._rankData = data;
  if (!data.length) {
    el.innerHTML = '<div class="empty">এই ফিল্টারে কোনো center নেই।</div>';
    return;
  }
              ...
              <td style="min-width:140px">
                ...
                <div style="display:flex;align-items:center;gap:8px">
                  ...
                  <span style="font-size:12px;font-weight:700;color:${color};min-width:36px">${toBn(pct)}%</span>
                    ...
                    : '<span style="font-size:11px;color:var(--muted);font-style:italic">লক্ষ্যমাত্রা নেই</span>'
                }
              </td>
            </tr>`;
            })
    ...
    <tbody>${sorted
      .map(
        (
          c,
          i,
        ) => `<tr style="${c === best ? "background:#0a2a1a" : c === worst ? "background:#2a0a0a" : ""}">
...
window._sortRanking = (type, btn) => {
  ...
  const sorted =
    type === "sales"
      ...
      : type === "production"
        ? [...data].sort((a, b) => b.total_produced - a.total_produced)
        : [...data].sort((a, b) => b.total_stock - a.total_stock);
  document.getElementById("rankingTableBody").innerHTML = rankTableHTML(
    sorted,
    type,
  );
...
window._exportCSV = () => {
  ...
  const rows = data.map((c) => [
    ...
    c.district || "",
    c.division || "",
    c.total_revenue.toFixed(0),
    c.current_month_rev.toFixed(0),
    c.last_month_rev.toFixed(0),
    c.growth_rate.toFixed(1),
    c.total_produced,
    c.total_stock,
    c.target_achieved !== null ? c.target_achieved.toFixed(1) : "N/A",
    c.perf_score,
    c.traffic_light === "green"
      ? "ভালো"
      : c.traffic_light === "yellow"
        ? "মাঝারি"
        : "দুর্বল",
  ]);
  ...
  a.href = url;
  a.download = `HortNet-BD_Report_${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
  ...
  try {
    ...
    if (id) {
      ...
      res = await fetch(`${API}/tenants/${id}`, {
        ...
        body: JSON.stringify({
          ...data,
          db_url: data.db_url || cur?.db_url || "",
          active: cur?.active ?? true,
        }),
      });
    } else {
      if (!data.db_url || !document.getElementById("fSlug").value.trim())
        return showToast("Slug ও DB URL দিন।");
...
async function toggleCenter(id) {
  try {
    const r = await fetch(`${API}/tenants/${id}/toggle`, {
      method: "POST",
      headers: { Authorization: "Bearer " + token },
    });
    const d = await r.json();
...
function renderAdminMgmt(el) {
  ...
  const roleColors = {
    director: "#7c3aed",
    deputy_director: "#059669",
    horticulturist: "#0284c7",
    nursery_supervisor: "#d97706",
  };
  ...
  el.innerHTML = `
    ...
    ${
      allAdmins.length === 0
        ? '<div class="empty"><i class="ti ti-users"></i>কোনো Admin নেই।</div>'
        : `<div class="center-list">
      ${allAdmins
        .map(
          (a) => `
...
function openAdminModal(id) {
  ...
  if (id) {
    ...
    document.getElementById("aLocation").value =
      nameParts.length > 1 ? nameParts.slice(1).join(", ") : a.name;
...
async function saveAdmin() {
  ...
  const data = {
    ...
    is_active: id
      ? document.getElementById("aIsActive").value === "true"
      : true,
  };
  if (!location || !data.email || !data.role)
    return showToast("স্থানের নাম, ইমেইল ও পদবী দিন।");
  if (!id && !data.password)
    return showToast("নতুন admin-এর জন্য পাসওয়ার্ড দিন।");
  try {
...
async function saveAssignments() {
  ...
  const checkboxes = document
    .getElementById("centerCheckboxes")
    .querySelectorAll("input[type=checkbox]:checked");
  const assigned_centers = [...checkboxes].map((cb) => cb.value);
  try {
    const r = await fetch(`${API}/admins/${adminId}/assignments`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + token,
      },
      body: JSON.stringify({ assigned_centers }),
    });
    const d = await r.json();
    if (!d.success) return showToast(d.message || d.error);
    showToast(d.message);
...
function fmt(n) {
  return toBn(parseInt(parseFloat(n || 0)).toLocaleString("en-IN"));
}
function fmtN(n) {
  return toBn(parseInt(n || 0).toLocaleString("en-IN"));
}
function fmtK(n) {
...
function tlLabel(t) {
  return t === "green" ? "ভালো" : t === "yellow" ? "সতর্ক" : "সমস্যা";
}
function tlBorderColor(t) {
  return t === "green" ? "#16a34a" : t === "yellow" ? "#d97706" : "#dc2626";
}
