...
router.post('/login', async (req, res) => {
    ...
    try {
        const r = await masterDb.query('SELECT * FROM super_admins WHERE email=$1', [email]);
        if (!r.rows.length) return res.status(401).json({ success: false, message: 'ইমেইল বা পাসওয়ার্ড ভুল।' });
        ...
        const token = jwt.sign({ id: r.rows[0].id, email: r.rows[0].email, name: r.rows[0].name }, SA_SECRET, { expiresIn: '8h' });
...
router.get('/tenants', saAuth, async (req, res) => {
    try {
        const r = await masterDb.query('SELECT id,slug,name_bn,name_en,location,currency,active,created_at FROM tenants ORDER BY created_at DESC');
        res.json({ success: true, data: r.rows });
    } catch (err) { res.status(500).json({ success: false, error: err.message }); }
});

// ===== ALL CENTER QUICK STATS =====
router.get('/stats-all', saAuth, async (req, res) => {
    try {
        const tenants = await getTenants();
        const results = await Promise.all(Object.entries(tenants).map(async ([slug, tenant]) => {
            try {
                const [sales, production, stock, today] = await Promise.all([
                    ...
                    queryTenant(tenant.db_url, `SELECT COALESCE(SUM(produced_quantity),0) AS total_produced FROM production_batches`),
                    queryTenant(tenant.db_url, `SELECT COALESCE(SUM(current_stock),0) AS total_stock, COALESCE(SUM(current_stock*unit_price),0) AS stock_value FROM seedlings WHERE is_active=true`),
                    queryTenant(tenant.db_url, `SELECT COALESCE(SUM(total_amount),0) AS today_revenue FROM sales WHERE sale_date=CURRENT_DATE`),
                ]);
                return { slug, name_bn: tenant.name_bn, name_en: tenant.name_en, location: tenant.location,
                    total_revenue: parseFloat(sales[0].total_revenue), total_invoices: parseInt(sales[0].total_invoices),
                    total_produced: parseInt(production[0].total_produced), total_stock: parseInt(stock[0].total_stock),
                    stock_value: parseFloat(stock[0].stock_value), today_revenue: parseFloat(today[0].today_revenue), status: 'ok' };
            } catch (e) { return { slug, name_en: tenant.name_en, status: 'error', error: e.message }; }
        }));
...
router.get('/center/:slug', saAuth, async (req, res) => {
    try {
        ...
        const [
            salesSummary, todaySales, monthlySales,
            productionSummary, productionByType,
            stockSummary, lowStock,
            damagesSummary, topSeedlings,
            recentSales, recentBatches,
            users, otherIncome,
            targets, categories
        ] = await Promise.all([
        ...
        const dmgTotal   = damagesSummary.reduce((s,r) => s + parseInt(r.total_damaged||0), 0) ||
                           (damagesSummary[0]?.total_damaged ? parseInt(damagesSummary[0].total_damaged) : 0);
        const dmgReports = damagesSummary.reduce((s,r) => s + parseInt(r.total_reports||r.count||0), 0) ||
                           parseInt(damagesSummary[0]?.total_reports||0);

        // অন্যান্য আয়ের মোট
        const totalOtherIncome = otherIncome.reduce((s,r) => s + parseFloat(r.total||0), 0);

        res.json({
            success: true,
            center: { slug: req.params.slug, name_bn: tenant.name_bn, name_en: tenant.name_en, location: tenant.location, currency: tenant.currency },
            sales: {
