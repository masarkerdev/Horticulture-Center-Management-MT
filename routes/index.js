...
router.get("/dashboard/stats", authenticate, fyMiddleware, async (req, res) => {
  try {
    ...
    const [sales, todaySales, prod, stock, damages, lowStock, recentSales] =
      await Promise.all([
        db.query(
        ...
        db.query(
          `SELECT COUNT(*) AS total_batches, COALESCE(SUM(produced_quantity),0) AS total_produced, COALESCE(SUM(success_quantity),0) AS total_success, COALESCE(SUM(failed_quantity),0) AS total_failed, COALESCE(AVG(CASE WHEN success_percent>0 THEN success_percent END),0) AS avg_success, COUNT(CASE WHEN status='active' THEN 1 END) AS active_batches FROM production_batches WHERE sowing_date BETWEEN $1 AND $2`,
          [fyStart, fyEnd],
        ),
        db.query(
...
router.get("/stock", authenticate, async (req, res) => {
  try {
    ...
    obData.rows.forEach((r) => {
      obMap[r.seedling_id] = parseInt(r.total_opening || 0);
    });
    const prodMap = {};
    prodData.rows.forEach((r) => {
      prodMap[r.seedling_id] = parseInt(r.total_produced || 0);
    });
    const saleMap = {};
    saleData.rows.forEach((r) => {
      saleMap[r.seedling_id] = parseInt(r.total_sale || 0);
    });
    const dmgMap = {};
    dmgData.rows.forEach((r) => {
      dmgMap[r.seedling_id] = parseInt(r.total_damage || 0);
    });
...
router.post("/other-income", authenticate, async (req, res) => {
  ...
  try {
    const result = await db.query(
      `INSERT INTO other_income (income_type,category,amount,income_date,description,created_by) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
      [
        income_type,
        category || null,
        amount,
...
router.put(
  ...
  adminOrManager,
  async (req, res) => {
    const { income_type, category, amount, income_date, description } =
      req.body;
    try {
      await db.query(
...
router.post("/employees-info", authenticate, adminOnly, async (req, res) => {
  ...
  if (!name_bn || !designation)
    return res.status(400).json({ success: false, message: "নাম ও পদবি দিন।" });
  try {
    const result = await db.query(
